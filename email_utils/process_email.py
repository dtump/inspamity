...
def get_email_content(source, is_string=False):
    """
    Extract content and metadata from an email file or string.
    
    Args:
        source: Either a file path or the email content as a string
        is_string: Boolean indicating if source is a string (True) or file path (False)
    ...
    for key in msg.keys():
        # Skip DKIM, Authentication/ARC and X- headers with long values
        if key.lower() in ['dkim-signature', 'arc-message-signature', 'domainkey-signature', 'arc-seal', 'arc-authentication-results', 'authentication-results', 'received-spf', 'mime-version']:
            continue
    ...
    # If there is a HTML body use that one even when there is a plain text body. Then convert HTML to text
    if html_content:
